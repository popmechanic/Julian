...
- **Same-value-both-workers secrets.** `SYNC_READ_SECRET` and
  `INTROSPECT_SECRET` are each a single value held identically on both
  `julian-broker` and `julian-sync` — generated once locally and piped,
  never printed, to both workers in the same shell (never two independent
  generations). Broker mints/sends its half of a request, sync compares —
  each secret's own row above lists which worker sends and which validates
...
| `SESSION_SECRET` | Sign approver sessions on the gate | T1 | Cloudflare worker secret on `julian-broker` | `https://gate.julian.soul.store` | `cd broker && bunx wrangler secret put SESSION_SECRET` (generate 64 random hex or base64url chars locally, Marcus types) | not yet rotated | active |
| `INTROSPECT_SECRET` | Verify introspection requests from sync and doors | T1 | Cloudflare worker secret, same value on both `julian-broker` (validates) and `julian-sync` (presents) | `https://gate.julian.soul.store` (validates), `https://sync.julian.soul.store` (presents) | one value generated locally, piped never printed to BOTH workers in one shell: `VAL=$(openssl rand -base64 32); (cd broker && printf '%s' "$VAL" \| bunx wrangler secret put INTROSPECT_SECRET); (cd sync && printf '%s' "$VAL" \| bunx wrangler secret put INTROSPECT_SECRET)` — never two independent generations (the 2B-pre mismatch lesson) | 2026-08-13 — first rotation, by Julian with Marcus present (B3 runbook step 2); value never printed; verified live by the introspect probe | active |
| `SYNC_READ_SECRET` | Authenticate internal read requests from broker to sync | T1 | Cloudflare worker secret, same value on both `julian-broker` (sends) and `julian-sync` (validates) | `https://sync.julian.soul.store` (validates), `https://gate.julian.soul.store` (sends) | one value generated locally, piped never printed to BOTH workers in one shell (same pattern as INTROSPECT_SECRET's row — never two independent generations) | 2026-08-13 — first install, by Julian with Marcus present (B3 runbook step 0); value never printed; verified by `wrangler secret list` on both workers | active |
| `GATE_BREAKGLASS_SECRET` | Admin break-glass path to lease list/revoke/export | T0 (Mac) + T1 (worker) | Mac `.env` + Cloudflare worker secret `BREAKGLASS_SECRET` on `julian-broker` | `https://gate.julian.soul.store` | Same value on both sides: generate locally, replace in Mac `.env`, then `cd broker && bunx wrangler secret put BREAKGLASS_SECRET` (same value) | not yet rotated | active |
