...
const SERVICES = [
  { name: 'julian-sync', purpose: 'TinyBase MergeableStore sync (Durable Object); the shared record all doors converge into.', endpoint: 'https://sync.julian.soul.store', auth: 'Delegated session lease via the gate\'s /exchange (browser tabs; 60s single-use socket tickets), door leases for /export (stream-read+); legacy Pocket ID JWTs were accepted only until the sunset ceremony (2026-08-25) and are no longer accepted.' },
  { name: 'julian-broker', purpose: 'Credential broker — doors get verbs, never keys. Mail verbs (send/list/read/health) for julian-marcus@agentmail.to with daily caps and an audit ledger (`bun scripts/mail-broker.ts --agent-doc`). Send gate applies: draft, show the human, wait. — the gate: doors authenticate with per-door leases (knock: `bun scripts/door-knock.ts`)', endpoint: 'https://gate.julian.soul.store', auth: 'Door lease (device-flow knock, Marcus approves; see door-knock.ts). Legacy Pocket ID bearers were accepted only during the migration window, which closed at the sunset ceremony (2026-08-25). Service keys held by the gate, never by agent or harness; browser sessions trade a Pocket ID login for scoped stream standing at /exchange' },
];
