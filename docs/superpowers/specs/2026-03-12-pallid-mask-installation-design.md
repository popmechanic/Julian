...
Nine states, linear progression. The ceremony module owns all transitions and is the only module aware of the overall flow.

```
WELCOME → SUMMON → PROMPT → INPUT → DIVINE → FORTUNE → REVEAL → QR_OFFER → QR_DISPLAY → WELCOME
```
...
| SUMMON | Mask animates in (fade/scale). Sigils begin. Pre-fetched greeting plays once mask is visible. | ElevenLabs TTS: Pallid Mask greets the visitor, frames the ritual. Generated by Claude from soul.md. | Audio complete |
| PROMPT | Mask continues speaking — asks visitor to formulate a question about past, present, or future. | ElevenLabs TTS: continuation of greeting, prompting the question. | Audio complete |
...
├── data/
│   ├── sigils.json        ← 200 sigil paths (extracted from sigils-all.js)
...
**ceremony.ts** — The brain. Manages the 9-state machine. Owns the keyboard listener and delegates meaning based on current state. Calls other modules' public methods (`mask.show()`, `sigils.loadingMode()`, `display.showNarration()`, etc.). No DOM manipulation of its own.
...
Returns audio as a buffer or URL the client can play.
